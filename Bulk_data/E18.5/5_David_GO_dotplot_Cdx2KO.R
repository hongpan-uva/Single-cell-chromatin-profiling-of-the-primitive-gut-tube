...
df1$Term <- term
df1$GeneType <- "Cdx2 KO-sepecific genes associated with Gained peak"
...
p1 <- ggplot(df1) + aes(x=GeneType,y=Term,colour=Benjamini,size=Count) + scale_colour_gradientn(colours=rev(customzied_pal),oob=scales::squish,trans="log10",limits=c(1e-5,1),name="Benjamini") + scale_size(limits=c(0,15),range = c(0, 15),breaks=c(15,10,5)) + geom_point() + guides(color = guide_colourbar(order=1), size = guide_legend(order=2)) + theme(axis.text.y = element_text(size = 12))
ggsave("Cdx2KO_genes_withGainedPeaks_GOterm_BP.pdf",height=12,width=12,p1)
